  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-4 py-8">
      <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-xl dark:border-slate-700 dark:bg-slate-900">
        <div className="mb-4">
          <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">Test Lead Mock Response</h3>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
            Configure mock buyer response for campaign #{card.displayId}: {card.name}
          </p>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Campaign min price: ${card.minPrice.toFixed(2)}. Timeout delays the mock response before returning the result.
          </p>
        </div>
          ...
          {isAccept ? (
            ...
            <div>
              ...
              <textarea
                ...
                className="mt-1 w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-slate-600 dark:bg-slate-950 dark:text-slate-100"
              />
