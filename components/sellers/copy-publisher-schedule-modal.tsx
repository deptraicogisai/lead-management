...
type CopyPublisherScheduleModalProps = {
  open: boolean;
...
export function CopyPublisherScheduleModal({
  open,
  sourceSellerId,
  mappingId,
  onClose,
  onApplied,
}: CopyPublisherScheduleModalProps) {
  ...
  useEffect(() => {
    ...
    const loadPublishers = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          toast.error("Failed to load publishers.", "Copy Schedule");
          return;
        }
  ...
  const handleApply = async () => {
    if (selectedPublisherIds.length === 0) {
      toast.error("Please select at least one publisher.", "Copy Schedule");
      return;
    }
    ...
    try {
      ...
      toast.success(
        data.message ?? `Schedule copied to ${data.updatedCount ?? selectedPublisherIds.length} publisher(s).`,
        "Copy Schedule"
      );
      onApplied?.();
      onClose();
  ...
  return (
    <Modal
      ...
      description="Apply will replace the current schedule of selected publishers with this publisher's schedule for the same product."
      onClose={onClose}
